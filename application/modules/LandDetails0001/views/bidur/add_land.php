...
$(document).ready(function() {
        ...
        var new_row =  '<tr class="nagadi_rasid_frm"><td>'+
            '<div class ="row">'+
                ...
                '<div class="col-md-2">'+
                    '<div class="form-group">'+
                        '<label>साबिक वडा नं<span style="color:red">*</span></label>'+
                        '<select name="old_ward[]" class="from-control dd_select oldward" id="old_ward" required><option value="">छान्नुहोस्</option></select>'+
                    '</div>'+
                '</div>'+
                '<div class="col-md-2">'+
                    '<div class="form-group">'+
            ...
            success: function(resp) {
                if (resp.status == 'success') {
                    if (resp.data == null) {
                        ...
                        obj.closest("tr").find('.max_rate').val(resp.tax_amount);
                        ...
                        var aana_amount = aana_rate * resp.tax_amount;
                        var paisa_amount = paisa_rate * resp.tax_amount;
                        var dam_amount = dam_rate * resp.tax_amount;
    ...
    $(document).on('input', '.ropani, .aana, .paisa, .dam', function() {
        ...
        var paisa = obj.closest("tr").find('.paisa').val();
        var dam = obj.closest("tr").find('.dam').val();
        var tax_rate = obj.closest("tr").find('.max_rate').val();
        var min_amount = obj.closest("tr").find('.min_rate').val();
        if (aana > 15) {
        ...
        var ropani = obj.closest("tr").find('.ropani').val()||0;
        var aana = obj.closest("tr").find('.aana').val()||0;
        var paisa = obj.closest("tr").find('.paisa').val()||0;
        var dam = obj.closest("tr").find('.dam').val()||0;
        
        if(kubul_rate > min_amount ) {
            var tax_amount_r = 300;
            console.log('here');
        } else {
            console.log('out');
            var tax_amount_r = tax_rate;
        }
        var ropani_amount = ropani * tax_amount_r;
        var aana_rate = aana / 16;
        var paisa_rate = paisa / 64;
        var dam_rate = dam / 256;      
        var total_ropani = parseFloat(ropani) + parseFloat(aana_rate) + parseFloat(paisa_rate) + parseFloat(dam_rate);
        var total_amount = Math.floor(total_ropani * tax_amount_r);
        if(total_amount < 0.5){
            var net_tax = 1;
        } else {
            net_tax = total_amount
        }
        obj.closest("tr").find('.tax_amount').val(tax_amount_r); 
    ...
    $(document).on('input', '.kubul_rate', function() {
        ...
        obj.closest("tr").find('.tax_amount').val(net_tax.toFixed());
    });
});
</script>
