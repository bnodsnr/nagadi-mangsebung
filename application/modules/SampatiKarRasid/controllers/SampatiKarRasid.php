...
class SampatiKarRasid extends MX_Controller
{
    public function __construct()
    {
        ...
        //$this->load->model("Profile/SampatiKarRasidModel");
    ...
    */
    public function SaveBillDetails() {
        if($this->input->is_ajax_request()) {
            ...
            if($result) {
                ...
                $this->SampatiKarRasidModel->updateLandDetails($nb_file_no, $land_flag); //update land flag
    ...
    public function printPreview($bill_no) {
        if($this->authlibrary->HasModulePermission($this->module_code, "VIEW")){
            $profile                        = $this->CommonModel->getwhere('sampati_kar_bhumi_kar_bill_details', array('bill_no' => $bill_no,'fiscal_year' => '2078/079'));
    ...
    */
    public function viewRasid($file_no){
        if($this->authlibrary->HasModulePermission($this->module_code, "VIEW")){
            ...
            // $profile                        = $this->CommonModel->getDataBySelectedFields('sampati_kar_bhumi_kar_bill_details', 'bill_no', $file_no);
       ...
       // if($this->authlibrary->HasModulePermission($this->module_code, "VIEW")){
            ...
            // $this->load->view('new/new_preview', $data);
        // } else {
        //     redirect('dashboard');
        // }
    }
    ...
    public function GetBills() 
    {
      if($this->input->is_ajax_request()) {
        $columns = array( 
          ...
          $totalData              = $this->SampatiKarRasidModel->CountBills($org_file_no,$bill_no,$from_date,$to_date, $status);
          $totalFiltered          = $totalData;
          $posts                  = $this->SampatiKarRasidModel->GetAllBills($limit,$start,$order,$dir, $org_file_no,$bill_no,$from_date,$to_date, $status);
    ...
    */
    ...
    public function save() {
        if($this->input->is_ajax_request()) {
            ...
            if($result) {
                ...
                $profile_data   = array('initial_flag' => 0);
                $land_data      = array('initial_flag' => 0, 'voucher_id'=> 0);
    ...
    public function previewCancelBill($bill_no) {
         if($this->authlibrary->HasModulePermission($this->module_code, "VIEW")){
            ...
            $data['user']                   = $this->CommonModel->getCurrentUser($this->session->userdata('PRJ_USER_ID'));
            $data['billcount']              = $this->SampatiKarRasidModel->totalSamptiKarBillDetails();
            $data['Billsdetails']           = $this->SampatiKarRasidModel->GetCancelBillDetails($fileNo);
