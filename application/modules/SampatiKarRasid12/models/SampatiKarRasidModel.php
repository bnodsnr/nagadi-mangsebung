...
class SampatiKarRasidModel extends CI_Model
{
    ...
    public function getSampatiKarAmount($rangePrice, $fiscal_year) {
        $sql = "
    ...
    public function getBhumiKarAmount($rangePrice, $fiscal_year) {
        //echo $rangePrice;die;
        $sql = "
    ...
    // public function getSampatiBhumiKarDetails($fileNo) {
    //     $sql = "SELECT SUM(net_total_amount) as total FROM sampati_kar_bhumi_kar_bill_details WHERE nb_file_no ='$fileNo'";
    //     return $this->db->get($sql)->row_array();
    // }
    
    public function getTotalBillAmountDetails($file_no){
        return $this->db->select('*')->from('sampati_kar_bhumi_kar_bill_details')
                                    ->where('nb_file_no', $file_no)
                                    ->get()
                                    ->row_array();
    }


    public function getTotalBillAmountDetailsByBillNo($bill_no){
        return $this->db->select('*')->from('sampati_kar_bhumi_kar_bill_details')
                                    ->where('bill_no', $bill_no)
                                    ->where('fiscal_year', '2078/079')
                                    ->get()
    ...
    */
    ...
    public function GetProfileDetailsForBills($file_no) {
        ...
        $sql = "SELECT 
                ...
                LEFT JOIN land_category lc ON lc.id = p.land_category
     ...
     * @return json
    */
    public function GetAllBills($limit,$start,$col,$dir, $org_file_no= NULL, $bill_no = NULL, $form_date,$to_date)
    {  $this->db->select('t1.*,t2.land_owner_name_np,t3.name')->from('sampati_kar_bhumi_kar_bill_details t1');
        ...
        //$this->db->where('t1.fiscal_year', '2078/079');
     ...
     * @return json
    */
    public function CountBills($org_file_no= NULL, $bill_no = NULL, $from_date =NULL,$to_date =NULL)
    {
